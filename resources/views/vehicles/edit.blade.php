...
<div class="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg mt-8">
    <h1 class="text-xl capitalize font-bold text-center mb-10">Editar Vehículo</h1>
    ...
    <form action="{{ route('vehicles.update', $vehicle->id) }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="date" name="date" id="date" value="{{ old('date', $vehicle->date) }}" 
                    class="w-full border border-gray-400 rounded-lg p-2 focus:ring-red-500 focus:border-red-500" required>
            </div>

            {{-- Vehículo --}}
            <div>
                <label for="vehicle" class="block text-gray-700 font-semibold mb-2">Vehículo</label>
                <input type="text" name="vehicle" id="vehicle" value="{{ old('vehicle', $vehicle->vehicle) }}"
                    class="w-full border border-gray-400 rounded-lg p-2 focus:ring-red-500 focus:border-red-500" required>
            </div>
            ...
            <div>
                ...
                <input type="text" name="plates" id="plates" value="{{ old('plates', $vehicle->plates) }}"
                    class="w-full border border-gray-400 rounded-lg p-2 focus:ring-red-500 focus:border-red-500" required>
            </div>
            ...
            <div>
                ...
                <input type="text" name="service_type" id="service_type" value="{{ old('service_type', $vehicle->service_type) }}"
                    class="w-full border border-gray-400 rounded-lg p-2 focus:ring-red-500 focus:border-red-500" required>
            </div>
            ...
            <div>
                ...
                <input type="text" name="order_number" id="order_number" value="{{ old('order_number', $vehicle->order_number) }}"
                    class="w-full border border-gray-400 rounded-lg p-2 focus:ring-red-500 focus:border-red-500" required>
            </div>
        </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                @foreach ($checkboxes as $name => $label)
                    <label class="flex items-center space-x-2 text-gray-600">
                        <input type="checkbox" name="{{ $name }}" id="{{ $name }}" {{ old($name, $vehicle->$name) ? 'checked' : '' }}
        ...
        <div class="mt-8">
            ...
            <textarea name="observations" id="observations" rows="4"
                class="w-full border border-gray-400 rounded-lg p-2 focus:ring-red-500 focus:border-red-500">{{ old('observations', $vehicle->observations) }}</textarea>
        </div>
